...
import {questionColumnLabel, QuestionColumnVisibility} from "@/types/adminQuestion";
...
type Props = {
    ...
    setColumnVisibility: React.Dispatch<SetStateAction<QuestionColumnVisibility>>
};

const QuestionColumnFilter = ({columnVisibility, setColumnVisibility}: Props) => {
        ...
        <DropdownMenu>
            <DropdownMenuTrigger asChild>
                <Button className={"relative pl-2 pr-8"} size={"sm"}>
                    {ad("questions_page.columns.filter_label")}
                ...
                {Object.keys(columnVisibility).map((key) => (
                    <DropdownMenuCheckboxItem
                        ...
                        onCheckedChange={(checked) => {
                            setColumnVisibility((prev: QuestionColumnVisibility) => ({
                                ...prev,
                                [key]: checked,
                            }));
                        }}
                    ...
                    >
                        {ad(questionColumnLabel[key as keyof typeof questionColumnLabel])}
                        {key === "contentEN" && (
...
export default QuestionColumnFilter;