...
import {OlyneQuestion, QuestionType} from "@/types/adminQuestion";
...
const UpdateQuestionForm = ({question, setIsOpen, setQuestion}: Props) => {
    const {t: ad} = useTranslation("admin");
    const {t: ts} = useTranslation("toast");
    const {isUpdatingQuestion, updateQuestion} = useAdminQuestionStore(
        (state) => ({
            isUpdatingQuestion: state.isUpdatingQuestion,
            updateQuestion: state.updateQuestion
        }),
        shallow
    );

    const form = useForm<UpdateQuestionFormValue>({
    ...
    const handleSubmit = async (data: UpdateQuestionFormValue) => {
        setIsOpen(false);
        const result = await updateQuestion(question.id, data);
        console.log(data)
        if (result.success) {
            toast.success(ts("admin.update_question.success"));
            setIsOpen(false);
            setQuestion(undefined);
        } else {
            toast.error(ts("admin.update_question.error"))
        }
    };
    ...
    return (
        <Form {...form}>
            <form onSubmit={form.handleSubmit(handleSubmit)} className="flex flex-col gap-4">
                ...
                >
                    {ad("questions_page.edit_question.submit_btn")}
                </Button>
            </form>
        </Form>
    )
