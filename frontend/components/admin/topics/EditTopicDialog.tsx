...
import EditTopicForm from "@/components/admin/topics/EditTopicForm";
...
const EditTopicDialog = ({topic, isOpen, setIsOpen, setTopic}: Props) => {
            ...
            <AlertDialogContent className={"w-full md:w-[600px]"}>
                <AlertDialogHeader className={"w-full items-center"}>
                    <AlertDialogTitle className={"text-xl font-medium text-fg-light-100 text-center"}>
                        {ad("questions_page.topics_tab.edit_topic.title")}
                        <Button
                            size={"lg"}
                            onClick={() => {
                                setIsOpen(false);
                ...
                <EditTopicForm setIsOpen={setIsOpen} topic={topic} setTopic={setTopic} />
                <AlertDialogFooter>
                </AlertDialogFooter>
            </AlertDialogContent>
        </AlertDialog>
